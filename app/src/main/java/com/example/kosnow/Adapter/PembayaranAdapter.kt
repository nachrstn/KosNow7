...
class PembayaranAdapter(
    private val mContext: Context?, list: List<Pembayaran>, list2: List<KamarIsi>
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val fucek: Long = pembayaran.tgl_pembayaran?.toLong() ?: 0
        holder.tgl.text = getDate(fucek, "dd/MM/yyyy")
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (snapshot: DataSnapshot in dataSnapshot.children) {
                    ...
                    if (kamar?.key.equals(pembayaran.id_kamar)) {
                        ...
                        {
                            val intent = Intent(mContext, CekStrukActivity::class.java)
            ...
            override fun afterTextChanged(s: Editable) {
                if (s.toString() != current) {
                    ...
                    val replaceable = String.format(
                        "[Rp,.\\s]", NumberFormat.getCurrencyInstance().currency.getSymbol(local)
                    )
    ...
    private fun deleteData(position: Int) {
        val builder: AlertDialog.Builder = AlertDialog.Builder(
            mContext!!
        ).setMessage("Do you want to delete this room?").setPositiveButton(
                "Yes"
            ) { dialog, which ->
                val database: FirebaseDatabase = FirebaseDatabase.getInstance()
                val pembayaran: Pembayaran = listPembayaran[position]
                ...
                cekKamarIsi.addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onDataChange(dataSnapshot: DataSnapshot) {
                        if (dataSnapshot.hasChild("Kamar_terisi")) {
                            ...
                            val myRef2: DatabaseReference? = kamarIsi.key?.let {
                                database.reference.child("Kamar_terisi").child(it)
                            }
                            if (myRef2 != null) {
                                myRef2.removeValue()
                            }
